...
assert.equal(policy.scope, 'Cartograph first-party theme source and repository tooling');
...
assert.deepEqual(
  ...
  [
    {
      rule: 'no-empty',
      observedDiagnostics: 50,
      observedAffectedFiles: 2,
      decision: 'accepted-no-action'
    },
    {
      rule: 'no-unused-vars',
      observedDiagnostics: 59,
      observedAffectedFiles: 2,
      decision: 'accepted-no-action'
    },
    {
...
assert.equal(eslintDebtBaseline.diagnostics.length, 109);
...
assert.deepEqual(htmlPolicy.wrappers, []);
