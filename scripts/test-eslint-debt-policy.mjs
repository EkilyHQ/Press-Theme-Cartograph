...
for (const filePath of ['theme/modules/interactions.js', 'theme/modules/layout.js', 'theme/modules/views.js']) {
  const cartographSource = await readFile(new URL(`../${filePath}`, import.meta.url), 'utf8');
  ...
  assert.deepEqual(
    createBaseline(scanJavaScriptSource({ filePath, source: cartographSource })),
    createBaseline(scanJavaScriptSource({ filePath, source: formattedCartographSource })),
    `the exact Cartograph debt baseline must remain stable when Prettier formats ${filePath}`
  );
}
