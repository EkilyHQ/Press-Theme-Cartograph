...
for (const filePath of ['theme/modules/interactions.js', 'theme/modules/layout.js', 'theme/modules/views.js']) {
  const cartographSource = await readFile(path.join(SCRIPT_DIR, '..', filePath), 'utf8');
  const formattedCartographSource = await prettier.format(cartographSource, {
    parser: 'babel',
    printWidth: 120,
    semi: true,
    singleQuote: true,
  ...
  assert.deepEqual(
    ...
    withoutLocations(
      scanJavaScriptSource({
        filePath,
        source: formattedCartographSource,
        wrapperNames: policy.wrappers.map(({ name }) => name)
      })
    ),
    `the exact Cartograph sink inventory must remain stable when Prettier formats ${filePath}`
  );
}
...
const wrapperBasePolicy = structuredClone(policy);
wrapperBasePolicy.wrappers = [
  {
    name: 'setHtml',
    rationale: 'Synthetic reviewed wrapper used only to prove that established wrapper names cannot disappear.'
  }
];
const removedWrapperPolicy = structuredClone(wrapperBasePolicy);
removedWrapperPolicy.wrappers = [];
assert.ok(
  verifyPolicyTransition(wrapperBasePolicy, removedWrapperPolicy).some((error) =>
    /wrapper removal or rename is forbidden/u.test(error)
  ),
  'an established wrapper name must not be removed from the head policy'
);
const renamedWrapperPolicy = structuredClone(wrapperBasePolicy);
renamedWrapperPolicy.wrappers[0].name = 'setMarkup';
assert.ok(
  verifyPolicyTransition(wrapperBasePolicy, renamedWrapperPolicy).some((error) =>
    /wrapper removal or rename is forbidden/u.test(error)
  ),
